...
Purpose:
    Perform tasks related to style classification
    Accepts a small list of input data in the order as input to Vitals object in the order [OG,FG,ABV,SRM,IBU] as the following form:
        {'OG':[min,max], 'FG':[min,max], 'ABV':[min,max], 'SRM':[min,max], 'IBU':[min,max]}
...
class Vitals(object):
	...
	def randomizeVitals(self):
                '''
                novelty function for changing the bounds of the 'vitals' to a random set of bounds, thereby simulating various style
                '''
	...
	def checkValidity(self,newvitals):
                '''
                Returns true if input vitals (newvitals) are wthin the bounds for the Vitals objec
                '''
	...
	def makeTrainingData(self,n=10,labels=[]):
                '''
                makes random training data that is within the proper vitals bounds - n = number of traning datas
                '''
